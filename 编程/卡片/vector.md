...
- 底层：堆上分配的连续内存数组，内部维护三个指针：
  - `start`：指向数组起始位置
  - `finish`：指向最后一个元素之后的位置（`size() = finish - start`）
  - `end_of_storage`：指向已分配空间的末尾（`capacity() = end_of_storage - start`）
- 扩容：当 `finish == end_of_storage` 时触发，分配新空间（GCC 2 倍 / MSVC 1.5 倍），拷贝/移动元素，释放旧空间
- `resize()` 改变 `size`（可能触发扩容），`reserve()` 只改变 `capacity`（不影响 size）
- 元素不能是引用（引用无地址，无法连续存储）
- 迭代器失效：插入导致扩容时全部失效，删除后当前位置及之后失效
- 迭代器支持随机访问，支持比较运算
