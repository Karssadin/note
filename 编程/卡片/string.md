...
- 底层：动态字符数组（类似 `vector<char>` 但提供了更丰富的字符串操作接口）
- 短字符串优化（SSO）：短字符串（通常 <=15 字节）直接存储在栈上的 string 对象内部，避免堆分配；超过阈值后才在堆上分配
- `c_str()` 返回以 `\0` 结尾的 C 风格字符串指针
- `size()` 和 `length()` 完全等价，都返回字符个数（不含 `\0`）
- `capacity()` 返回已分配空间大小，`reserve()` 可预分配，减少扩容次数

## 基本概念

- C++ 风格的字符串，string 本质是一个类
- 与 `char*` 的区别：`char*` 是指针，`string` 是类，内部封装了 `char*` 并负责内存管理
- 不用担心赋值越界、取值越界等问题，内部自动处理
